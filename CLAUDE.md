...
| `scripts/check-space-sync.mjs` | Read-only drift report: Zen spaces ↔ containers ↔ the route table. `npm run check:spaces`. Reads the live Zen profile (copies sqlite + WAL first, never writes). No browser or daemon needed. |
