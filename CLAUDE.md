...
- **Every input event is untrusted, and no WebExtension API changes that.** `pointerClick`, `hoverElement`, `pressKeys` and friends build events in JS and `dispatchEvent` them, so pages see `isTrusted: false` and the browser grants no user activation. Consequences a caller will hit: a click that opens a tab/window is popup-blocked (route around it with the link's `href` + `open_url`); clipboard, fullscreen, file pickers and audible autoplay are unreachable; `hover` never sets CSS `:hover`; `press_key` fires the page's listeners only — no typing, no `Tab` focus move, no `Enter` submit, no browser shortcuts. The README's *Input is synthetic* table is the user-facing version; keep both in step. **Don't describe this MCP as interacting "like a person"** — that belief is what made a popup-blocked `View` button look like a bug in 2026-09. Known self-inflicted gap, not yet fixed: `pointerClick` calls `focus()` even when the page cancelled `pointerdown`/`mousedown`, which a real browser wouldn't, and it still fires `mousedown`/`mouseup` after a cancelled `pointerdown`, which the Pointer Events spec says to suppress.
