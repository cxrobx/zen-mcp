...
- **zen-mcp picks the CONTAINER; Zen picks the SPACE — and only Space Routing joins them without stealing focus.** There is no WebExtension API for spaces, so a tab this MCP opens is stamped with whatever space is active at that moment (`_shouldShowTab` assigns any tab lacking `zen-workspace-id` to the current one). Zen offers two ways to close that gap and **they are not equivalent.** **Space Routing** rules (`zen-space-routing.jsonlz4`, Zen 1.21+) map URL→space and are applied in `ZenSpaceRoutingManager.#routeToWorkspace`, which moves the tab but gates the space *switch* on `!inBackground` — we open in the background, so the tab is filed and focus stays put. That is the mechanism to use. **`zen.workspaces.force-container-workspace` is a trap: do not enable it.** It files by container correctly, but `ZenSpaceManager.onTabBrowserInserted` follows every move with an unconditional `setTimeout(() => this.changeWorkspace(workspace), 0)` — no `inBackground` check — so each background tab an agent opens yanks the user's browser to another space; with several sessions driving different projects it never stops jumping. Measured against live Zen 1.21.15b on 2026-08-25 and reverted; the profile's `user.js` pins it `false` with that reasoning. Note routing rules also run *after* the force logic and outrank it, so a rule that disagrees with `containers.json` wins and the tab lands in the right space wearing the wrong container. Zen never overrides a container we asked for (`tabbrowser.js` substitutes its own only when `userContextId` is `undefined`). Verify with `npm run check:spaces`. **Spaces live in `zen-sessions.jsonlz4` (mozlz4, `spaces[].containerTabId`), NOT in `zen_workspaces` in places.sqlite** — that table is a one-time migration source `ZenSessionManager` reads once on the first launch of Zen 1.21+ and never writes again, so it is a fossil that still lists deleted spaces and misses every space created since. Reading it produced a confidently wrong report here (4 of 6 containers "unbound" when all 6 were fine); don't go back to sqlite when a space looks missing.
